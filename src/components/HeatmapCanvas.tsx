 ...
 * The heatmap uses radial gradients at each detected hotspot and the verdict
 * tone determines the color (red for fake, amber for suspicious, green for safe).
 */
export const HeatmapCanvas = ({ src, isVideo, result, showHeatmap }: Props) => {
  ...
  // Track displayed media size so the canvas overlay aligns perfectly.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!canvas || !result || !showHeatmap || dims.w === 0) {
      if (canvas) {
    ...
    // Draw bounding boxes + labels in source-over so they appear above heat
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden rounded-lg border border-border bg-black">
      {isVideo ? (
        <video src={src} controls className="block w-full" />
      ) : (
        <img src={src} alt="Media being analyzed for deepfake artifacts" className="block w-full" />
      )}
      <canvas
        ref={overlayRef}
        className="pointer-events-none absolute inset-0 transition-opacity duration-300"
        style={{ opacity: showHeatmap && result ? 1 : 0 }}
      />
    </div>
  );
};
